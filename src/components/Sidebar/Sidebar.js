...
const Sidebar = () => {
  ...
  // const [showExportDropdown, setShowExportDropdown] = useState(false); // Commented out - no longer using dropdown
  const [showTemplateSelector, setShowTemplateSelector] = useState(false);
  const [changingTemplateFor, setChangingTemplateFor] = useState(null);

  const createNewProposal = () => {
    setChangingTemplateFor(null);
    setShowTemplateSelector(true);
  };

  const handleChangeTemplate = (proposal, event) => {
    event.stopPropagation(); // Prevent triggering the proposal selection
    setChangingTemplateFor(proposal);
    setShowTemplateSelector(true);
  };
  ...
  const handleTemplateSelect = (selection) => {
    ...
    if (changingTemplateFor) {
      ...
      const updatedBranding = selectedTemplate || {
        colors: {
          primary: '#007bff',
          secondary: '#6c757d',
          accent: '#28a745'
      ...
      const newProposal = {
        ...
        sections: [
          ...
          { id: uuidv4(), type: 'fees', title: 'Fees', content: '', order: 2 },
  ...
  // Commented out PDF export functionality
  /*
  ...
  return (
    <SidebarContainer>
      ...
      <Section>
        ...
        {/* Export Button Group - Commented out dropdown, keeping only DOCX export */}
        <Button
        ...
        {/* Commented out export dropdown with multiple options
        <ExportDropdown>
          <DropdownButton 
        ...
        {proposals.map(proposal => (
          ...
          >
            ...
            <ChangeTemplateButton
              onClick={(e) => handleChangeTemplate(proposal, e)}
      ...
      <TemplateSelector
        ...
        mode={changingTemplateFor ? 'change' : 'create'}
      />
    </SidebarContainer>
  );
};
